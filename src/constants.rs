use crate::error_response::error_responses::{
    ErrorResponse, ALREADY_REGISTERED_EMAIL_JSON, ERROR_NOT_FOUND_STATUS, ERROR_UNAUTHORIZED_STATUS,
    ...
    WRONG_REQUEST_JSON, ERROR_WRONG_NICKNAME_STATUS, WRONG_NICKNAME_JSON, ERROR_ALREADY_NICKNAME_STATUS, ALREADY_REGISTERED_NICKNAME_JSON, ERROR_ALREADY_REGISTERED_STATUS,
};
...
//min && max len login_nickname
pub const LEN_NICK_NAME: LenText = LenText { min: 2, max: 100 };
...
pub const WRONG_NICKNAME: (Status, Json<ErrorResponse>) =
    (ERROR_WRONG_NICKNAME_STATUS, Json(WRONG_NICKNAME_JSON));

pub const ALREDY_REGISTERED_NICKNAME: (Status, Json<ErrorResponse>) = (
    ERROR_ALREADY_NICKNAME_STATUS,
    Json(ALREADY_REGISTERED_NICKNAME_JSON),
);
